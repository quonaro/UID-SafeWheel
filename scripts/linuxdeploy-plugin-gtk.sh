...
# Vendored from https://github.com/tauri-apps/linuxdeploy-plugin-gtk (MIT),
# with local fixes marked "uid-quiz":
#  - export WEBKIT_DISABLE_* in the AppRun hook (WebKitGTK in AppImage)
#  - tolerate missing gdk-pixbuf loader dir / cache files (Arch, minimal CI)
#
...
EOF
        ...
        # uid-quiz: don't abort when the query tool was not found
        [ -f "$APPDIR/${gtk3_immodules_cache_file/$LD_GTK_LIBRARY_PATH//usr/lib}" ] && \
...
# uid-quiz: gdk-pixbuf ships no external loaders on some distros (e.g. Arch);
# keep the target dir so loaders.cache can still be generated
if [ -d "$gdk_pixbuf_binarydir" ]; then
    copy_lib_tree "$gdk_pixbuf_binarydir" "$APPDIR/"
else
    mkdir -p "$APPDIR/${gdk_pixbuf_binarydir/$LD_GTK_LIBRARY_PATH//usr/lib}"
fi
cat >> "$HOOKFILE" <<EOF
